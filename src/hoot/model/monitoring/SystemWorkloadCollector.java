...
public class SystemWorkloadCollector implements CollectorInterface
{
        ...
        try {
            ...
            while ((line = outputReader.readLine()) != null) {
                if (line.contains("Class (")) {
                    ...
                    newVariableStorage.put(MEMORY_CLASS_RESERVED, res[0]);
                    newVariableStorage.put(MEMORY_CLASS_COMMITTED, res[1]);
                } else if (line.contains("(classes")) {
                    String tmp = line.split("#")[1].stripTrailing();
                    tmp = tmp.substring(0, tmp.length() - 1);
                    try {
                        newVariableStorage.put(MEMORY_CLASS_CLASSES, Integer.parseInt(tmp));
                    } catch (NumberFormatException ignore) {
                    }
                } else if (line.contains("Thread")) {
                    int[] res = getReservedAndCommitted(line);
                    newVariableStorage.put(MEMORY_THREAD_RESERVED, res[0]);
                    newVariableStorage.put(MEMORY_THREAD_COMMITTED, res[1]);
                } else if (line.contains("Code")) {
                    int[] res = getReservedAndCommitted(line);
                    newVariableStorage.put(MEMORY_CODE_RESERVED, res[0]);
                    newVariableStorage.put(MEMORY_CODE_COMMITTED, res[1]);
                } else if (line.contains("GC")) {
                    int[] res = getReservedAndCommitted(line);
                    newVariableStorage.put(MEMORY_GC_RESERVED, res[0]);
                    newVariableStorage.put(MEMORY_GC_COMMITTED, res[1]);
                }
            }

            return newVariableStorage;
        } catch (IOException e) {
    ...
    public synchronized CollectorResult collect() throws CollectorException
    {
        ...
        {{
            ...
            Map<String, Integer> memoryInfo = getMemoryInfo();
            if (memoryInfo != null) {
                putAll(memoryInfo);
            }
        }};
